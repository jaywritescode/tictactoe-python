...
class BoardState:
    ...
    def successor(self, player_name, row, col):
        succ = tuple(tuple(player_name if (r, c) == (row, col) else self[r,c] for c in range(3)) for r in range(3))
...
class MinimaxState(State):
    ...
    def __init__(self, ply, name, next_player, board):
        ...
        board_tuple = tuple(tuple(board[row][col] for col in range(3)) for row in range(3))
    ...
    def successors(self):
        ...
        return [successor(row, col) for row in range(3) for col in range(3) if self.board[row, col] is None]